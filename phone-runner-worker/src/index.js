...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/wake") {
      return handleManualWake(request, env);
    }
    if (request.method === "POST" && url.pathname === "/register") {
      return handleDeviceRegistration(request, env);
    }
    if (request.method === "POST" && url.pathname === "/report") {
      return handlePhoneReport(request, env);
    }
...
async function handleGithubWebhook(request, env) {
  ...
  if (!await verifyGithubSignature(raw, signature, env.GITHUB_WEBHOOK_SECRET)) {
  ...
  if (payload?.repository?.full_name !== EXPECTED_REPOSITORY) {
    return json({ ok: true, ignored: "repository" });
  }
  ...
  if (!touched) {
    return json({ ok: true, ignored: "path" });
  }
...
async function handleManualWake(request, env) {
  if (!env.WAKE_API_TOKEN) {
    return new Response("Manual wake is not configured", { status: 503 });
  }

  const auth = request.headers.get("authorization") || "";
  if (!constantTimeEqual(auth, `Bearer ${env.WAKE_API_TOKEN}`)) {
...
async function handleDeviceRegistration(request, env) {
  ...
  if (!authorizedWithRegistrationToken(request, env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  ...
  if (pairedDeviceId && pairedDeviceId !== deviceId) {
    return new Response("A different phone is already paired", { status: 409 });
  }

  await env.DEVICE_STATE.put(DEVICE_TOKEN_KEY, token);
  await env.DEVICE_STATE.put(DEVICE_META_KEY, JSON.stringify({
    device_id: deviceId,
    model,
...
async function handlePhoneReport(request, env) {
  ...
  const pairedDeviceId = await getPairedDeviceId(env);
  if (!pairedDeviceId) {
    return new Response("No paired phone", { status: 409 });
  }
  if (!deviceId || deviceId !== pairedDeviceId) {
    return new Response("Wrong phone", { status: 409 });
  }

  const phase = safeMetadata(body?.phase, 40);
  if (!REPORT_PHASES.has(phase)) {
    return new Response("Invalid report phase", { status: 400 });
  }
  ...
  await env.DEVICE_STATE.put(PHONE_STATUS_KEY, JSON.stringify({
...
async function handleHealth(env) {
  ...
  if (env.DEVICE_STATE && typeof env.DEVICE_STATE.get === "function") {
    ...
    if (phoneStatusRaw) {
      try {
        phoneStatus = JSON.parse(phoneStatusRaw);
      } catch (_) {
        phoneStatus = null;
      }
    }
  ...
  if (!registered && env.FCM_DEVICE_TOKEN) {
    registered = true;
    tokenSource = "secret-fallback";
  }
  ...
  return json({
    ...
    phone_exit_code: Number.isInteger(phoneStatus?.exit_code)
      ? phoneStatus.exit_code
      : null,
  });
}
...
async function sendWake(env, requestId, source) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`FCM ${response.status}: ${text.slice(0, 500)}`);
  }
}
...
async function getDeviceToken(env) {
  ...
  if (env.FCM_DEVICE_TOKEN) {
    return env.FCM_DEVICE_TOKEN;
  }
...
async function getPairedDeviceId(env) {
  ...
  try {
    ...
    throw new Error("Device registry metadata is invalid");
  }
}

function registrationConfigError(env) {
  if (!env.DEVICE_REGISTRATION_TOKEN) {
    return new Response("Device registration is not configured", { status: 503 });
  }
  if (
    !env.DEVICE_STATE
    || typeof env.DEVICE_STATE.get !== "function"
    || typeof env.DEVICE_STATE.put !== "function"
  ) {
...
function authorizedWithRegistrationToken(request, env) {
  const auth = request.headers.get("authorization") || "";
  return constantTimeEqual(auth, `Bearer ${env.DEVICE_REGISTRATION_TOKEN}`);
}
...
async function getAccessToken(env) {
  ...
  if (!response.ok) {
    throw new Error(`OAuth ${response.status}: ${(await response.text()).slice(0, 500)}`);
  }
